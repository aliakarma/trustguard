  ...
  return (
    <div className="flex flex-col gap-6 animate-fade-in-up">
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-5 gap-4">
        <PilotStat label="Duration" value={String(p.duration_days)} unit="days" />
        <PilotStat label="Volunteer Devices" value={String(p.devices)} unit="phones" />
        <PilotStat label="Median Apps" value={String(p.median_apps_per_device)} unit="apps" />
        <PilotStat label="Android Range" value={`API ${p.android_versions}`} valueSize="1.55rem" />
        <PilotStat label="IRB Review" value="Approved" valueSize="1.55rem" tone="safe" />
      </div>
      ...
      <GlassPanel accentTop enforce className="p-6">
        <h3 className="stat-label mb-4 text-danger">Sim-to-Real Calibration Gap</h3>
        ...
        <div className="flex flex-col md:flex-row gap-6 items-center">
          <div className="flex-1 flex flex-col gap-3">
            ...
            <div className="bg-red-500/10 border border-red-500/20 text-danger p-4 rounded-xl text-xs font-bold mt-2">
              ⚠ WARNING: The pilot study was run in notification-only mode. Actual automated revocation was disabled in the field.
            </div>
          </div>

          <div className="flex gap-6 items-center bg-surface p-6 rounded-xl border border-subtle">
            <div className="flex flex-col items-center gap-1">
              <span className="text-[10px] text-tertiary uppercase">Sim Target</span>
              <div className="w-16 h-16 rounded-full border-2 border-emerald-500 flex items-center justify-center font-mono font-bold text-safe text-lg">
                2.5%
              </div>
            </div>
            ...
            <div className="flex flex-col items-center gap-1">
              ...
              <div className="w-16 h-16 rounded-full border-2 border-red-500 flex items-center justify-center font-mono font-bold text-danger text-lg" style={{ boxShadow: '0 0 10px rgba(239, 68, 68, 0.2)' }}>
                3.4%
              </div>
            </div>
          </div>
      ...
      <GlassPanel accentTop constraint className="p-6">
        <h4 className="stat-label mb-4">Takeaways &amp; recalibration needs</h4>
        <ul className="flex flex-col gap-3">
          {findings.map((f, idx) => (
            <li key={idx} className="flex items-start gap-2.5 text-sm text-secondary leading-relaxed">
              <span className="h-1.5 w-1.5 rounded-full bg-constraint mt-2 flex-shrink-0" />
              <span>{f}</span>
            </li>
          ))}
        </ul>
...
function PilotStat({
  ...
  return (
    <GlassPanel accentTop constraint className="p-5 flex flex-col justify-between gap-3" style={{ minHeight: '112px' }}>
      <span className="stat-label">{label}</span>
      <div className="flex items-baseline gap-1.5">
        <span className="stat-value" style={{ fontSize: valueSize, color: tone === 'safe' ? 'var(--accent-safe)' : 'var(--text-primary)' }}>
          {value}
        </span>
        {unit && <span className="text-xs text-tertiary font-medium">{unit}</span>}
      </div>
    </GlassPanel>
  );
}
