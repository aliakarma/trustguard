...
export default function SensitivityStudio() {
  ...
  return (
    <div className="flex flex-col gap-6 animate-fade-in-up">
      {/* ── Tabs Navigation ─────────────────────────────────────────── */}
      <div className="seg self-start overflow-x-auto max-w-full">
        {([
          ['sensitivity', 'Hyperparameter Sensitivity'],
          ['ablation', 'Ablation Studies'],
          ['dynamics', 'Constraint Dynamics'],
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
      ...
      <GlassPanel accentTop monitor className="p-5 flex flex-col justify-between">
                ...
                {Object.entries(colData).map(([lambda2Key, metrics]) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <span className={`text-[10px] font-mono ${frrExceeded ? 'text-danger font-bold' : 'text-safe'}`}>
                        FRR: {metrics.FRR_pct}%
                      </span>
                    </div>
                  );
        ...
        <div className="text-[10px] text-tertiary bg-surface p-3 rounded-lg border border-subtle mt-4">
      ...
      <GlassPanel accentTop enforce className="p-5 col-span-1 lg:col-span-2">
        ...
        <div className="grid grid-4 gap-4 mt-4">
          {Object.entries(emaAlphaResults).map(([alphaKey, metrics]: [string, any]) => (
            <div key={alphaKey} className="bg-surface/40 border border-subtle p-4 rounded-xl flex flex-col justify-between">
    ...
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              ...
              <div
                ...
                className={`p-4 rounded-xl border flex flex-col justify-between gap-3 ${
                  isOurs
                    ...
                    : isViolated
                    ? 'border-red-500/20 bg-red-500/5'
                    : 'border-subtle bg-surface/20'
                }`}
              >
                <div>
                  <span className={`text-[10px] font-bold uppercase tracking-wider ${isOurs ? 'text-monitor' : 'text-tertiary'}`}>
                    {name}
                  </span>
                ...
                <div className="flex justify-between items-center border-t border-subtle pt-2">
                  ...
                  <span className={`text-xs font-mono font-bold ${isViolated ? 'text-danger' : 'text-safe'}`}>
      ...
      <div className="lg:col-span-2">
        <GlassPanel accentTop constraint className="p-6">
          <h4 className="stat-label mb-4">Ablation takeaways</h4>
              ...
              return (
                <div key={iter} className="flex justify-between items-center border-b border-subtle pb-3 last:border-0 last:pb-0">
                  ...
                  <div className="flex items-center gap-6">
                    <div className="flex flex-col text-right">
                      <span className={`font-mono font-bold ${frr > safeLimit ? 'text-danger' : 'text-safe'}`}>
                        {frr.toFixed(1)}%
                      </span>
        ...
        <div>
          ...
          <div className="flex flex-col gap-4 mt-4">
            <div className="flex justify-between border-b border-subtle pb-3">
              ...
              <span className="font-mono text-safe font-bold">{categoryFrr.min.FRR_pct}%</span>
            </div>
            <div className="flex justify-between border-b border-subtle pb-3">
              <span className="text-xs text-secondary">Maximum Category: <strong>{categoryFrr.max.category}</strong></span>
              <span className="font-mono text-danger font-bold">{categoryFrr.max.FRR_pct}%</span>
            </div>
